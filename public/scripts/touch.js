...
jQuery.fn.touch = function(settings) {
	...
	settings = jQuery.extend({
		animate: true, sticky: false, dragx: true, dragy: true,
...
function touchmove(e) {
	...
	if(_dragging && !_sizing && _animate) {
		var _lastleft = (isNaN(parseInt(t.css("left")))) ? 0 : parseInt(t.css("left"));
		var _lasttop = (isNaN(parseInt(t.css("top")))) ? 0 : parseInt(t.css("top"));
	}
	$(e.changedTouches).each(function(){
		e.preventDefault();
		...
		_left = (this.pageX - (parseInt(t.css("width")) / 2)) - $(".board").offset().left;
		_top = (this.pageY - (parseInt(t.css("height")) / 2)) - $(".board").offset().top;
		...
		if(_dragging && !_sizing) {
			...
			if(_dragx || _dragy) t.css({ position: "absolute" });
			if(_dragx) t.css({ left: _left + "px" });
			if(_dragy) t.css({ top: _top + "px" });
		}
	});
};
function touchend(e) {
